...
def test_get_utterances_subset_should_work():
    # Given
    utterances = [
    ...
    utterances_subset = sorted(
        utterances_subset, key=lambda u: "%s%s" % (u[0], u[1]["data"][0]["text"])
    )
    ...
    assert expected_utterances == utterances_subset


def test_update_entities_with_utterances():
    # Given
    dataset = {
        "intents": {
            "intent_1": {
                "utterances": [
                    {
                        "data": [
                            {"text": "aa", "entity": "entity_2"},
                            {"text": "bb", "entity": "entity_2"},
                        ]
                    }
                ]
            },
            "intent_2": {
                "utterances": [{"data": [{"text": "cccc", "entity": "entity_1"}]}]
            },
        },
        "entities": {
            "entity_1": {"data": [], "use_synonyms": False},
            "entity_2": {
                "data": [{"value": "a", "synonyms": ["aa"]}],
                "use_synonyms": True,
            },
        },
    }
    ...
    assert expected_dataset == updated_dataset
